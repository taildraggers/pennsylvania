"""Aggregates Pennsylvania-located listings from every taildraggers.com
...
ones whose location ends in ", PA".
...
repos' approach, with the state filter swapped to "PA".
"""
...
PAGE_TITLE = "Taildragger Ads in Pennsylvania"
STATE_ABBR = "PA"
...
_PENNSYLVANIA_LOCATION_RE = re.compile(r",\s*PA\s*$", re.IGNORECASE)


def _is_pennsylvania(location: str) -> bool:
    return bool(_PENNSYLVANIA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        pennsylvania_rows = [row for row in rows if _is_pennsylvania(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(pennsylvania_rows)} in Pennsylvania")
        all_listings.extend(pennsylvania_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Pennsylvania listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Pennsylvania listings: {len(listings)}")
