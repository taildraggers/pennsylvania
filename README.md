...
Daily aggregator of Pennsylvania-located taildragger classified
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, PA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Pennsylvania"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/pennsylvania/`
...
```html
<iframe
  src="https://taildraggers.github.io/pennsylvania/"
  title="Taildragger Ads in Pennsylvania"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_PENNSYLVANIA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
